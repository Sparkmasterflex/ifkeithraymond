class Image < ActiveRecord::Base
  ...
  has_attached_file :attachment,
    :path          => ":rails_root/public/images/uploaded/:id_:style.:extension",
    :url           => "/images/uploaded/:id_:style.:extension",
    :styles        => lambda { |attachment| ContentType::IMAGES.include?(attachment.instance_read(:content_type)) ? { :thumb => ["80x80>", :png], :home => ["195x275>", :png], :about => ["580x434>", :png], :preview => ["850x612>", :png] } : {} },
    :default_style => :small
...
#  after_update :move_imported_scan
  ...
  def features
    options = []
    feats = self.attachable_type.constantize.find(:first, :conditions => {:id => self.attachable_id}).features
    feats.each { |f| options << [f.feature, f.id]}
