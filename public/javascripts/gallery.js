...
var Gallery = Class.create({
  ...
  showPopup: function() {   
    ...
    console.log(this.pos);
      ...
      //var pos = centerPosition(this.temp.down('img'));

      // scale the content window to the correct size
      if(this.window.down('img')) new Effect.Fade(this.window.down('img'), {duration: 0.25});

      new Effect.Parallel([
        new Effect.Morph(this.window, {style: {height: (this.pos.height + 35) + "px", width: this.pos.width + "px"}}),
        new Effect.Move(this.window, {x: this.pos.left, y: this.pos.top, mode: 'absolute'})
...
// returns results as key/value pairs of an object
function centerPosition(el, width, height) {
